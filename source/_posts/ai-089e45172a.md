...
source_url: "https://www.infoq.cn/article/mKFB15kPm6Uw8fNG4Y7e?utm_source=rss&utm_medium=article"
---
...
-   Async Agent API（即将 generally available）：让长任务在后台执行，而不需要一直保持连接打开；
    
-   Code Execution Tool（即将进入 public preview）：在沙箱化 Python 环境里处理数据并生成高质量输出；
    
-   Interrupt and Resume（即将 generally available）：中断一个正在运行的 agent、修正方向，然后从中断点继续执行；
    
-   Partial Access（即将进入 public preview）：用同一个 managed agent 服务不同权限等级的用户。
...
![agent request debugger](/ai-knowledge-qoder/_imgs/5242a331a91c42df.png)

图 1：“使用 CoCo 进行调试”——一款基于托管代理 API 和 CoCo 运行时构建的工程师工具。
...
![cortex](/ai-knowledge-qoder/_imgs/9b6b75b0d2e8f86f.png)
...
-   试用 Code Execution Tool，并[基于相应 sandbox configuration 与 package setup 搭建自己的流程](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents/code-execution)；
    
-   [用声明式编排指南及配套示例](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents/multi-agent)，构建你的第一个 multi-agent workflow；
    
-   [查看 Snowflake Summit 2026 的 demos](https://www.snowflake.com/en/summit/)，了解这些能力在真实环境中的运行方式。
    

原文地址：[https://www.snowflake.com/en/blog/snowflake-cortex-agents-enterprise-ai-scale/](https://www.snowflake.com/en/blog/snowflake-cortex-agents-enterprise-ai-scale/ "xxx")
...
> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/mKFB15kPm6Uw8fNG4Y7e?utm_source=rss&utm_medium=article）。