...
tags:
  - AI
  - InfoQ 中文
excerpt: "当大多数模型公司还在围绕“是否开放权重”争论时，一个总参数达到 5350 亿的大模型，选择在训练尚未完成、甚至仍可能中途失败的时候，把训练曲线、数据配方、模型配置和技术讨论直接放到了网上。 近日，斯坦"
source_url: "https://www.infoq.cn/article/y7KTOS9YbBz0OcoyiweQ?utm_source=rss&utm_medium=article"
---
...
![](/ai-knowledge-qoder/_imgs/98e2ef947e2cc809.png)
...
![](/ai-knowledge-qoder/_imgs/fc25ce7f314ede6e.png)

David Hall

![](/ai-knowledge-qoder/_imgs/ab248dd79a575adc.png)
...
![](/ai-knowledge-qoder/_imgs/8232cd28a6fedeb8.png)

所以 Marin 提出一种“开放实验室”机制：每个实验都通过 GitHub Issue 提前声明目标和假设，具体配置以代码和 Pull Request 提交，外部研究者可以参与 Review，实验启动后，W&B 训练指标公开。更重要的是，在这个过程中所有成功、失败和中途修改痕迹都被记录，数据、代码、配方及最终模型继续开放。
...
![](/ai-knowledge-qoder/_imgs/97d5a30f5b676a57.png)

不过，Marin 目前还不是一个已经完成并可供比较的新模型。

这场实验真正引发关注的原因，不是 5350 亿参数本身，而是它把通常被模型公司严密保护的训练过程，变成了一个可以实时观察和审查的公开研究对象。

## 535B 并不意味着每次都要运行 535B 参数
...
![](/ai-knowledge-qoder/_imgs/d9306194adfa9508.png)

不过，“230 亿激活参数”也不能简单等同于一款 23B 稠密模型。
...
![](/ai-knowledge-qoder/_imgs/87d10ef4a23c8320.png)

因此，Marin 535B 没有一开始就追求超长上下文，而是退回 4K 上下文启动预训练。
...
![](/ai-knowledge-qoder/_imgs/401f8a02ab6837e0.png)

Marin 在公开记录中表示，由于没有找到在 JAX/XLA GPU 环境中性能足够好的现成专家并行方案，团队自行实现了 EP，也就是 Expert Parallelism。
...
![](/ai-knowledge-qoder/_imgs/dd6dd842c9e52c64.png)

Marin 表示，这套缩放实验只占最终计算量的约 1%，但能够承担几项关键任务。
...
![](/ai-knowledge-qoder/_imgs/e835424a31759054.png)

此后的 Pythia、LLM360 和 Ai2 的 OLMo 系列，也进一步开放训练数据、代码、日志和中间检查点。OLMo 的论文明确将 Pythia 和 BLOOM 列为此前开放程度最高的模型项目，并表示 OLMo 进一步发布了从数据处理到训练、评估的完整框架。
...
[https://github.com/marin-community/marin/issues](https://github.com/marin-community/marin/issues)

[https://marin.community/blog/2025/05/19/announcement/](https://marin.community/blog/2025/05/19/announcement/)


---

> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/y7KTOS9YbBz0OcoyiweQ?utm_source=rss&utm_medium=article）。