...
tags:
  - AI
  - InfoQ 中文
excerpt: "通过一个个看似合理的变更——它们虽然能通过所有既定规则的检验，但其实现却在悄无声息地偏离团队原本认为已经加以保护的设计意图。从历史经验来看，解决之道是人工审查。问题在于，人工审查无法扩展到每个拉取请求"
source_url: "https://www.infoq.cn/article/8iWWrov7bkk1hz5CcFft?utm_source=rss&utm_medium=article"
---
...
![](https://imgopt.infoq.com/fit-in/3000x4000/filters:quality\(85\)/filters:no_upscale\(\)/articles/agentic-fitness-functions-evolutionary-architecture/en/resources/236figure-1-1786430482601.jpg)
...
![](https://imgopt.infoq.com/fit-in/3000x4000/filters:quality\(85\)/filters:no_upscale\(\)/articles/agentic-fitness-functions-evolutionary-architecture/en/resources/178figure-2-1786430482601.jpg)
...
为了具体说明这一模式，我们基于 ADK 创建了一个小型参考实现：[agent-fitness-functions](https://github.com/vijayk85/agentic-fitness-functions)。
...
![](https://imgopt.infoq.com/fit-in/3000x4000/filters:quality\(85\)/filters:no_upscale\(\)/articles/agentic-fitness-functions-evolutionary-architecture/en/resources/146figure-3-1786430482601.jpg)
...
1.  确保硬门槛具有确定性。对于能够以规则、模式、阈值、策略或可重复性实验等形式表达的条件，应当采用确定性检查。切勿用概率性判断取代可靠的门槛。
    

1.  从提建议开始。将每个新的智能体适应度函数先在建议模式下运行，直到校准可以证明其信号稳定、有用且值得采取行动为止。
    

1.  将证据范围限定在变更范围内。向智能体提供 PR 差异、涉及的契约、本地依赖关系图、相关的 ADR 以及一个较小的跟踪窗口，而非整个代码库。
    

1.  使用分析性评分标准。将判断分解为命名标准。单凭架构质量评分无法付诸行动；而标准层面的判定则可以。
    

1.  保留人工干预机制。当出现置信度不足、评审员意见不一致、影响范围过大或权衡取舍不明确等情况时，应该立即转交人工评审员进行处理。
    

1.  促进结果的稳定性。当特定的结果反复出现，且团队一致认为其代表真实的违规情况时，应将其转化为明确的评分标准或确定性规则。
    

1.  对评审流程进行版本管理。将提示词、评分标准、校准集、模型版本和工具定义视为受管控的成果。
    

1.  审核证据链。保存证据引用、判定结果、置信度、理由及后续行动，以便团队日后复核该决策。
...
原文链接：[https://www.infoq.com/articles/agentic-fitness-functions-evolutionary-architecture/](https://www.infoq.com/articles/agentic-fitness-functions-evolutionary-architecture/)


---

> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/8iWWrov7bkk1hz5CcFft?utm_source=rss&utm_medium=article）。