...
![](/ai-knowledge-qoder/_imgs/2c315b34cff9ff19.png)
...
CoCo 会把详细使用遥测写入各个 surface 对应的 ACCOUNT\_USAGE 视图：
...
每一行代表一次请求，其中包含 `TOKEN_CREDITS`、总 `TOKENS`，以及在 `TOKENS_GRANULAR` 和 `CREDITS_GRANULAR` 中记录的按模型拆分的 input、output 和 cache tokens 明细。`USER_ID`、`USER_TAGS` 和 `METADATA`（例如角色名、推理区域）字段则提供了归因和分摊能力。
...
![](/ai-knowledge-qoder/_imgs/a34c7ad4f1c35a78.png)
...
![](/ai-knowledge-qoder/_imgs/ca24d932ee2c4705.png)
...
-   Quota 还可以按 tags 定义作用范围，并用 `UNION` 或 `INTERSECTION` 逻辑组合选择用户。
...
![](https://miro.medium.com/v2/resize:fit:1050/1*tm7hpm2f8iF2fup-xaGG8w.gif)
...
-   `Account-level allowlist`：也就是历史上的 `CORTEX_MODELS_ALLOWLIST` 参数，可配置为 `All`、`None`，或者一个以逗号分隔的小写模型名列表。
...
原文地址：[https://medium.com/snowflake/7-ways-to-control-your-snowflake-coco-ai-spend-91fe829fce82](https://medium.com/snowflake/7-ways-to-control-your-snowflake-coco-ai-spend-91fe829fce82 "xxx")

![](/ai-knowledge-qoder/_imgs/719fbc08d82a13b0.jpg)

点击链接立即报名注册：[Ascent-Snowflake Platform Training-China](https://www.snowflake.com/events/snowflake-ascent-china/)更多 Snowflake 精彩活动请关注[专区](https://www.infoq.cn/space/snowflake)


---

> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/pOfV96f9DHG9Cw1KQAZB?utm_source=rss&utm_medium=article）。