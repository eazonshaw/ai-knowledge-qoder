...
> 
> 目前，柯南 AI 在快手内部大前端研发中的用户渗透率已接近 **60%**，根因分析结果的完整采纳率稳定在 **50% 左右**。在一个只有系统堆栈的鸿蒙 C++ Crash 案例中，它通过寄存器、业务日志和源码构建证据链，约 **10 分钟**便定位到内部基础组件触发的纯系统堆栈 Use-After-Free 问题，并帮助团队找到止损开关。围绕这一案例，王泽锋进一步拆解了柯南 AI 的五层架构，以及稳定性专家如何从“前线救火员”转变为“消防系统设计师”。

![](/ai-knowledge-qoder/_imgs/73f78a61f0128039.png)
...
当代码生产与稳定性处置长期速率不对等，线上 Case 的数量、严重程度和处置时延都可能全面劣化。更棘手的是，AI 不仅放大研发吞吐，也可能制造新的稳定性问题。由于缺乏全局上下文，AI 往往沿着“最小阻力路径”让问题现象消失，但现象消失不等于问题被真正解决。这就是 AI 普遍存在的**局部最优陷阱**。
...
![](/ai-knowledge-qoder/_imgs/3198800a863225ea.png)
...
![](/ai-knowledge-qoder/_imgs/ab9a14d25b35b320.png)
...
目前，柯南 AI 在内部大前端研发中的用户渗透率接近 **60%**，根因分析结果的完整采纳率稳定在 **50% 左右**。

## 柯南 AI 的五层工程架构

![](/ai-knowledge-qoder/_imgs/c5040743bebc16a9.png)
...
![](/ai-knowledge-qoder/_imgs/8c1952cb7d76c0ad.png)
...
![](/ai-knowledge-qoder/_imgs/c43ec0da89a66c5f.png)
...
在外围框架之外，我们投入最多的是**用 Skill 承接稳定性专家经验**。

![](/ai-knowledge-qoder/_imgs/cbcf965dcadd9786.png)
...
![](/ai-knowledge-qoder/_imgs/077faca95cdb2cb8.png)
...
![](/ai-knowledge-qoder/_imgs/bec92171a3d016d2.png)
...
![](/ai-knowledge-qoder/_imgs/bf174fbd1fa268de.png)
...
![](/ai-knowledge-qoder/_imgs/5f7fa5f6cab4e662.png)
...
![](/ai-knowledge-qoder/_imgs/c7a510443435022f.png)
...
![](/ai-knowledge-qoder/_imgs/d22d898cc335731a.png)
...
![](/ai-knowledge-qoder/_imgs/2004860ebf24c564.png)
...
1.  **确定性的归代码，概率性的归模型。** 组合使用 Workflow、单次 LLM 请求和 Agent，避免过度 AI 化，才能保证效果稳定与成本可控。
    
2.  **Agent Harness 的技术选型应当轻量化、领域化。** 基于成熟可靠的 Agent SDK，通过 Skill 承载专家经验，不必重复建设通用底座。
    
3.  **数据服务的关键是提供 Agent 友好的接口。** Bash 是一种有效形态，因为大模型天然熟悉其语义，但必须配套权限与安全边界。
    
4.  **上下文决定 Agent 的效果上限。** 端侧基建决定 Agent 能看到什么，也决定它能够分析到什么程度。
    
5.  **反馈迭代决定 Agent 的长期表现。** 只有持续运行 Bad Case 驱动的闭环，才能不断提高系统效果。
...
**王泽锋，**快手大前端稳定性方向技术专家。2023 年加入快手，先后参与 Android & 鸿蒙稳定性方向监控报警、排障工具、止损工具等多个领域的建设，目前主要聚焦探索 AI + 稳定性方向。
...
> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/rlXYaPUjVR0fffbQK3Iz?utm_source=rss&utm_medium=article）。