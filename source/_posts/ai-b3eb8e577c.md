...
source_url: "https://www.infoq.cn/article/2NsA9FT1uhjmdRrwzOo3?utm_source=rss&utm_medium=article"
---
...
![](/ai-knowledge-qoder/_imgs/574d73fb3b8f8fb7.png)
...
-   KG\_NODE 表存储所有实体，包括唯一标识符、类型（药物、疾病、细胞）和属性；属性通常保存在灵活的 VARIANT 列中。
    
-   KG\_EDGE 表存储关系，包括源节点 ID、目标节点 ID、边类型（例如 treats、targets、expressed\_in），以及时间戳或置信度分数等可选元数据。
...
![](/ai-knowledge-qoder/_imgs/a1c10a7ae4e8d56b.png)
...
-   针对每种智能体配置运行包含 22 个问题的评估套件
    
-   分析多次运行中的失败模式
    
-   提出并应用提示词改进，例如为 GraphRAG 加入显式的组织类型透传指令
...
![](/ai-knowledge-qoder/_imgs/de59e381efaea6da.png)
...
1.  **先修复数据层。**在本次基准中，在不使用静态映射的情况下，GraphRAG 配合聚合后代属性，已经取得了相对基线总体增益中的大部分，约为 80%。预先计算组织类型列表，是本次基准中影响最显著的干预之一。在投入人工整理规则之前，应先确保索引能够为每个概念提供完整数据。
    
2.  **工具更少，同样可以获得良好结果。**在本次基准中，扁平化 GraphRAG 智能体的表现与拥有 7 个工具的知识图谱智能体相当。更多工具能够提供更强的原始能力，但也扩大了可能发生编排错误的范围。GraphRAG 的简洁架构——一个搜索工具加一个 SQL 工具——缩小了智能体的决策空间，并产生了本次基准中最低的观测方差。GraphRAG 智能体的多次运行结果波动最小；硬编码映射方案虽然峰值准确率更高，但运行间方差也最高。
    
3.  **有针对性的硬编码可以弥合最后的差距。**术语映射智能体相较基线 GraphRAG 配置的优势，主要集中在“internal organ lining”“carcinoma-like models”这类无法仅通过层级遍历推导出的复合术语上。对于已知且范围有限的词表，在智能体提示词中嵌入人工整理的映射是有效的。这一层应保持精简：它应当补充索引，而不是取代索引。
    
4.  **Cortex Code 的优化技能让这套工作流可以重复执行。**构建本体感知智能体是一个迭代过程。Cortex Code 的智能体优化技能提供了一套“评估—优化—比较”循环，能够把架构选择转化为可衡量的准确性增益。在我们的结果中，最关键的设计决策——搜索文本富化、组织类型透传和阈值规则，都来自这一优化过程。**任何把本体导入 Snowflake 的团队，都可以遵循同样的流程：加载本体；构建索引；创建智能体；再使用 Cortex Code 进行优化。**
...
原文地址：[https://www.snowflake.com/en/blog/engineering/ontology-grounded-cortex-agents/](https://www.snowflake.com/en/blog/engineering/ontology-grounded-cortex-agents/ "xxx")


---

> 本文正文由程序自动抓取自公开网页/RSS，版权归原作者与来源站点所有；如有侵权请联系删除。原文出处：InfoQ 中文（https://www.infoq.cn/article/2NsA9FT1uhjmdRrwzOo3?utm_source=rss&utm_medium=article）。